...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    cal << %(<caption class="#{options[:month_name_class]}"></caption><thead><th colspan="7">#{Date::MONTHNAMES[options[:month]] + ' ' + options[:year].to_s}</th></tr><tr class="#{options[:day_name_class]}">)
    day_names.each do |d|
